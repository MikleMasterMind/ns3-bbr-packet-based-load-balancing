...
int main (int argc, char *argv[])
{
  ...
  //        | 10.1.1.0/24
  //        |
  // [Балансировщик (10.1.1.2)]
  //        /    |    |    \
  //       /     |    |     \
  // 10.1.2.0/30  |    |     10.1.2.12/30
  //     /        |    |        \
  // [R1]       [R2]  [R3]       [R4] (плохой путь)
  // 10.1.2.1  10.1.2.5 10.1.2.9  10.1.2.13
  //    |         |         |         |
  // 10.1.3.0/30 |         10.1.3.8/30 10.1.3.12/30
  //    |         |         |         |
  // 10.1.3.1  10.1.3.5  10.1.3.9  10.1.3.13
  //    |         |         |         |
  // [Сервер (10.1.4.1 на всех интерфейсах)]
  // 
  // Балансировщик имеет 4 маршрута к сети 10.1.4.0/24 через разные интерфейсы
  // Сервер имеет один адрес 10.1.4.1 на всех 4 интерфейсах
  // Маршрутизаторы R1-R4 подключены к серверу через подсети 10.1.3.x/30
  // ==========================================================================

  // Логирование для отладки
  LogComponentEnable ("PerPacketLoadBalancerExperiment", LOG_LEVEL_ALL);
  LogComponentEnable ("PerPacketLoadBalancer", LOG_LEVEL_ALL);
  ...
  uint32_t badPathIndex = -1;           // Индекс "плохого" пути (нумерация с 0)
  ...
  // Балансировщик распределяет пакеты между всеми путями
  ...
  // Добавляем маршрут к сети сервера через балансировщик
  clientRouting->AddNetworkRouteTo(Ipv4Address("10.1.4.0"), 
                                  Ipv4Mask("255.255.255.0"),
                                  Ipv4Address("10.1.1.2"),  // Адрес балансировщика
                                  1);  // Интерфейс подключенный к балансировщику

  NS_LOG_INFO("Добавлен маршрут на клиенте: 10.1.4.0/24 -> 10.1.1.2");
  ...
  // ==========================================================================
  // ВКЛЮЧЕНИЕ IP FORWARDING НА МАРШРУТИЗАТОРАХ
  // ==========================================================================
  NS_LOG_INFO ("Включение IP forwarding на маршрутизаторах...");
  for (uint32_t i = 0; i < numPaths; i++)
  {
    Ptr<Ipv4> routerIpv4 = routerNodes.Get(i)->GetObject<Ipv4>();
    for (uint32_t interface = 0; interface < routerIpv4->GetNInterfaces(); interface++)
    {
      routerIpv4->SetForwarding(interface, true);
    }
    NS_LOG_INFO("Включен IP forwarding на маршрутизаторе " << i);
  }

  // ==========================================================================
  // НАСТРОЙКА СТАТИЧЕСКОЙ МАРШРUTИЗАЦИИ НА МАРШРУТИЗАТОРАХ
  // ==========================================================================
